...
def download_stac_assets(
    ...
    """
    Download STAC assets concurrently.

    Parameters:
    -----------
    assets_to_download : list of dict
        List of asset info dicts with 'item_id', 'asset_key', 'url' keys
    ...
    --------
    dict
        Download results with success/failure counts and file info
    """
    import os
    import time
    ...
    def download_single_asset(asset_info):
        try:
            response = requests.get(asset_info['url'], stream=True)
            response.raise_for_status()

            # Generate filename
            filename = f"{asset_info['item_id']}_{asset_info['asset_key']}.tif"
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_asset = {
            executor.submit(download_single_asset, asset): asset
...
def download_stac_assets(
    ...
    -----------
    assets_to_download : list of dict
        List of asset dictionaries with 'item_id', 'asset_key', 'url' keys
    download_directory : str or Path
        Directory to save downloaded files
    max_workers : int, default=4
    ...
    --------
    dict
        Download results including success/failure counts and timing
    """
    from concurrent.futures import ThreadPoolExecutor, as_completed
    import time
    from pathlib import Path
...
def process_stac_timeseries(
    ...
    """
    Process STAC assets as a timeseries analysis.

    Parameters:
    -----------
    asset_paths : list of str or Path
        List of paths to raster assets
    analysis_type : str, default='basic_stats'
    ...
    if not asset_paths:
        raise ValueError("No asset paths provided")
